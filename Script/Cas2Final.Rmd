...
```{r outliers, cache=TRUE, include=T}
...
A partir del análisis exploratorio realizado de las series y considerando sus características se procedió a estimar 5 modelos de pronóstico lineales por cada serie:
...
Para el caso de la serie de los saldos del Activo Neto Administrado de los Fondos de Inversión del Mercado de Dinero en **colones**, se estimó un modelo autoregresivo de medias moviles (ARIMA) con transformación logarítmica, la cual tiene la siguiente forma:

En el gráfico xxx se puede apreciar el pronostico sobre los últimos 5 meses reservado para probar el modelo, es importante resaltar que los meses pronosticados se acercan en gran medida a los valores reales y los intervalos de confianza son amplios pero contienen los valores reales, para mas detalles sobre las metricas de rendimientos de los modelos dirigirse al anexo xxx.

Es importante resaltar que el modelo ARIMA()() cumple con los supuestos:

```{r **Modelo Holt Winter Multiplicativo**}
...
```{r **Modelo Holt Winter Aditivo**, results = FALSE}
...
```{r metricas holt winter crc rendimiento, results = FALSE}
...
```{r arima crc seas model, eval=T, echo=F, include=T}
## x13 ARIMA SEAS
#################################################
...
```{r arima crc residuos estacionariedad, include=T}
...
```{r arima crc residuos autocorrelacion, eval=T, include=T}
...
```{r arima crc residuos varianza constante arch, eval=T, include=T}
...
```{r arima crc residuos normalidad, include=T}
...
```{r rendimientoproncrc , fig.cap="Raíz del Error Cuadratico Medio de los Modelos Lineales según set de datos (entrenamiento y prueba) para la serie de los saldos del Activo Neto Administrado de los Fondos de Inversión del Mercado de Dinero en Colones", include=T}
...
```{r metricas de rendimiento lineales generales, include=T}
...
Por otro lado para la serie de los saldos del Activo Neto Administrado de los Fondos de Inversión del Mercado de Dinero en **dolares**, se estimó un modelo autoregresivo de medias moviles (ARIMA) con transformación logarítmica, la cual tiene la siguiente forma:
...
```{r arima usd seas model, eval=T, echo=F, include=T}
## x13 ARIMA SEAS
#################################################
## Ajustar modelo con x13arima seats
...
```{r arima usd residuos estacionariedad, include=T}
...
```{r arima usd residuos autocorrelacion, include=T}
#### Autocorrelaciòn de los resiudos
#####################################
#Autocorrelacion de los residuales y pruebas gráficas
## Ljung-Box test
...
```{r arima usd residuos normalidad, include=T}
#####################################
#Normalidad de los residuales
...
```{r arima usd residuos varianza constante arch, eval=T, include=T}
#### Varianza Constante de los residuos
## Varianza Constante ARCH Engle's Test for Residual Heteroscedasticity
# H0: los residuos son homocedasticos
# H1: los residuos no son homocedasticos
...
```{r tar ajustes,eval=T, include=T}
...
```{r Modelos Seleccionados, include=T}
...
```{r Full Training Forecast Dataset}
...
```{r MODELTIME WORKFLOW modeltime table, include=T}
...
```{r Measure Test Accuracy, include=T}
...
```{r Visualize test forecast, include=T}
...
```{r Refit on full training dataset}
submodels_refit_tbl <- submodels_calibrated_tbl %>%
  modeltime_refit(data_prepared_tbl)
```

```{r Visualize Submodel Forecast, include=T}
...
```{r Generate Resample Predictions }
...
```{r Evaluate the Results, include=T}
resamples_fitted %>%
...
```{r Ensemble test Accuracy, include=T}
...
```{r Ensemble Test Forecast, include=T}
ensemble_tbl%>%
  modeltime_calibrate(testing(splits))%>%
  modeltime_forecast(
    new_data =  testing(splits),
...
```{r Visualize Ensemble Forecast, include=T}
ensemble_refit_tbl%>%
  modeltime_forecast(
    new_data =  future_tbl,
    actual_data = data_prepared_tbl,
...
```{r MODELTIME WORKFLOW USD ###### modeltime table}
submodels_tbl <- modeltime_table(
  wflw_fit_prophet,
  wflw_fit_prophet_boost, #2
  #wflw_fit_xgboost_0_015, #3
...
```{r Measure Test Accuracy USD, include=T}
submodels_calibrated_tbl_train%>% 
  modeltime_accuracy()%>%
  arrange(rmse)%>%
  knitr::kable()
...
```{r Visualize test forecast USD, include=T}
...
```{r Refit on full training dataset USD}
submodels_refit_tbl <- submodels_calibrated_tbl %>%
  modeltime_refit(data_prepared_tbl)
```

```{r Visualize Submodel Forecast USD,include=T}
...
```{r Generate Resample Predictions USD, include=T}
resamples_fitted <- submodels_tbl %>%
    modeltime_fit_resamples(
        resamples = resamples_tscv,
        control   = control_resamples(verbose = FALSE)
...
```{r Evaluate the Results USD, include=T}
resamples_fitted %>%
    plot_modeltime_resamples(
      .point_size  = 3, 
      .point_alpha = 0.8,
...
```{r Ensemble test Accuracy USD, include=T}
...
```{r Ensemble Test Forecast USD, include=T}
ensemble_tbl%>%
  modeltime_calibrate(testing(splits))%>%
  modeltime_forecast(
    new_data =  testing(splits),
...
```{r Refit Ensemble USD}
ensemble_refit_tbl <- ensemble_tbl%>%
  modeltime_refit(data_prepared_tbl)
```

```{r Visualize Ensemble Forecast USD, include=T}
ensemble_refit_tbl%>%
  modeltime_forecast(
    new_data =  future_tbl,
    actual_data = data_prepared_tbl,
